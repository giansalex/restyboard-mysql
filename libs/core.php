...
function insertActivity($user_id, $comment, $type, $foreign_ids = array() , $revision = null, $foreign_id = null)
{
    ...
    if (!empty($foreign_ids['board_id']) || !empty($foreign_ids['organization_id']) || !empty($foreign_ids['user_id'])) {
        ...
        $res = pdoInsert($db_lnk, 'INSERT INTO activities (' . implode(', ', $fields) . ') VALUES (' . $val . ')', $values);
        $id = $res['id'];
    }
...
function sendMail($template, $replace_content, $to, $reply_to_mail = '')
{
    ...
    $templates = executeQuery('SELECT * FROM email_templates WHERE name = ?', $qry_val_arr);
    if ($templates) {
...
function copyCards($cards, $new_list_id, $name, $new_board_id = '')
{
    ...
    foreach ($cards as $card) {
        ...
        if ($card_result) {
            ...
            $attachments = pdoQueryFetch($db_lnk, 'SELECT id, ' . $attachment_fields . ' FROM card_attachments WHERE card_id = ? ORDER BY id', $qry_val_arr);
            ...
            $comments = pdoQueryFetch($db_lnk, 'SELECT id, ' . $comment_fields . ' FROM activities WHERE card_id = ? AND type = ? ORDER BY id', $qry_val_arr);
            if (count($comments) > 0) {
            ...
            $checklists = pdoQueryFetch($db_lnk, 'SELECT id, ' . $checklist_fields . ' FROM checklists WHERE card_id = ? ORDER BY id', $qry_val_arr);
            if (count($checklists)) {
                foreach ($checklists as $checklist) {
                    ...
                    if ($checklist_result) {
                        ...
                        $checklist_items = pdoQueryFetch($db_lnk, 'SELECT id, ' . $checklist_item_fields . ' FROM checklist_items WHERE checklist_id = ? ORDER BY id', $qry_val_arr);
            ...
            $cards_labels = pdoQueryFetch($db_lnk, 'SELECT id, ' . $cards_label_fields . ' FROM cards_labels WHERE card_id = ? ORDER BY id', $qry_val_arr);
            ...
            $cards_users = pdoQueryFetch($db_lnk, 'SELECT id, ' . $cards_user_fields . ' FROM cards_users WHERE card_id = ? ORDER BY id', $qry_val_arr);
...
function getbindValues($table, $data)
{
    ...
    $result = pdoQueryStm($db_lnk, 'SELECT * FROM information_schema.columns WHERE table_name = ?', $qry_val_arr);
    $bindValues = array();
    while ($field_details = $result->fetch(PDO::FETCH_ASSOC)) {
...
function importTrelloBoard($board = array())
{
    ...
    if (!empty($board)) {
        ...
        $new_board = pdoInsert($db_lnk,'INSERT INTO boards (created, modified, name, background_color, background_picture_url, background_pattern_url, user_id, board_visibility) VALUES (now(), now(), ?, ?, ?, ?, ?, ?)', $qry_val_arr);
        ...
        if (!empty($board['members'])) {
            foreach ($board['members'] as $member) {
                ...
                if (!$userExist) {
                    ...
                    $user = pdoInsert($db_lnk, 'INSERT INTO users (created, modified, role_id, username, email, password, is_active, is_email_confirmed, initials, full_name) VALUES (now(), now(), 2, ?, \'\', ?, true, true, ?, ?)', $qry_val_arr);
                ...
                pdoInsert($db_lnk, 'INSERT INTO boards_users (created, modified, user_id, board_id, board_user_role_id) VALUES (now(), now(), ?, ?, ?)', $qry_val_arr);
            }
        }
        $qry_val_arr = array(
            $authUser['id'],
        ...
        if (!empty($board['lists'])) {
            ...
            foreach ($board['lists'] as $list) {
                ...
                $_list = pdoInsert($db_lnk, 'INSERT INTO lists (created, modified, name, board_id, position, user_id, is_archived) VALUES (now(), now(), ?, ?, ?, ?, ?)', $qry_val_arr);
        ...
        if (!empty($board['cards'])) {
            foreach ($board['cards'] as $card) {
                ...
                $_card = pdoInsert($db_lnk, 'INSERT INTO cards (created, modified, board_id, list_id, name, description, is_archived, position, due_date, user_id) VALUES (now(), now(), ?, ?, ?, ?, ?, ?, ?, ?)', $qry_val_arr);
                    ...
                    foreach ($card['labels'] as $label) {
                        if (!empty($label['name'])) {
                            ...
                            if (empty($check_label)) {
                                ...
                                $check_label = pdoInsert($db_lnk, 'INSERT INTO labels (created, modified, name) VALUES (now(), now(), ?)', $qry_val_arr);
                            }
                            $qry_val_arr = array(
                                $new_board['id'],
                                $lists[$card['idList']],
                            ...
                            pdoInsert($db_lnk, 'INSERT INTO cards_labels (created, modified, board_id, list_id, card_id, label_id) VALUES (now(), now(), ?, ?, ?, ?)', $qry_val_arr);
                        }
                    }
                }
                if (!empty($card['attachments'])) {
                    foreach ($card['attachments'] as $attachment) {
                        ...
                        pdoInsert($db_lnk, 'INSERT INTO card_attachments (created, modified, board_id, list_id, card_id, name, path, mimetype) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', $qry_val_arr);
                    }
                }
                if (!empty($card['idMembersVoted'])) {
                    foreach ($card['idMembersVoted'] as $votedMemberId) {
                        ...
                        pdoInsert($db_lnk, 'INSERT INTO card_voters (created, modified, card_id, user_id) VALUES (now(), now(), ?, ?)', $qry_val_arr);
                    }
                }
                if (!empty($card['idMembers'])) {
                    foreach ($card['idMembers'] as $cardMemberId) {
                        ...
                        pdoInsert($db_lnk, 'INSERT INTO cards_users (created, modified, card_id, user_id) VALUES (now(), now(), ?, ?)', $qry_val_arr);
                    }
                }
            }
        }
        if (!empty($board['checklists'])) {
            ...
            foreach ($board['checklists'] as $checklist) {
                ...
                $_checklist = pdoInsert($db_lnk, 'INSERT INTO checklists (created, modified, name, position, card_id, user_id) VALUES (now(), now(), ?, ?, ?, ?)', $qry_val_arr);
                ...
                if (!empty($checklist['checkItems'])) {
                    foreach ($checklist['checkItems'] as $checkItem) {
                        ...
                        pdoInsert($db_lnk, 'INSERT INTO checklist_items (created, modified, name, position, card_id, checklist_id, is_completed, user_id) VALUES (now(), now(), ?, ?, ?, ?, ?, ?)', $qry_val_arr);
                    }
                }
            }
        }
        if (!empty($board['actions'])) {
            ...
            foreach ($board['actions'] as $action) {
                ...
                if (empty($lists_key) && empty($cards_key)) {
                    ...
                    pdoInsert($db_lnk, 'INSERT INTO activities (created, modified, board_id, user_id, type, comment) VALUES (?, ?, ?, ?, ?, ?)', $qry_val_arr);
                    ...
                    pdoInsert($db_lnk, 'INSERT INTO activities (created, modified, board_id, list_id, user_id, type, comment) VALUES (?, ?, ?, ?, ?, ?, ?)', $qry_val_arr);
                } else if (empty($lists_key) && !empty($cards_key)) {
                    $qry_val_arr = array(
                        $created,
                        $modified,
                    ...
                    pdoInsert($db_lnk, 'INSERT INTO activities (created, modified, board_id, card_id, user_id, type, comment) VALUES (?, ?, ?, ?, ?, ?, ?)', $qry_val_arr);
                } else if (!empty($lists_key) && !empty($cards_key)) {
                    $qry_val_arr = array(
                        $created,
                        $modified,
                    ...
                    pdoInsert($db_lnk, 'INSERT INTO activities (created, modified, board_id, list_id, card_id, user_id, type, comment) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', $qry_val_arr);
                }
            }
        }
        return $new_board;
...
function convertBooleanValues($table, $row)
{
    ...
    $stm = $db_lnk->prepare('SELECT * FROM information_schema.columns WHERE table_name = ?');
    $stm->execute($qry_val_arr);
    while ($field_details = $stm->fetch(PDO::FETCH_ASSOC)) {
...
{
    global $r_debug, $db_lnk, $authUser, $_server_domain_url;
    $c_result = pdoQueryStm($db_lnk, $c_sql, $pg_params);
    $count = $c_result->fetchColumn();
    ...
    $total_page = !empty($page_count) ? ceil($count / $page_count) : 0;
    $showing = (($start + $page_count) > $count) ? ($count - $start) : $page_count;
    $_metadata = array(
...
function getActivitiesObj($obj)
{
    ...
    if ($obj['type'] === 'add_board_user') {
        ...
        $s_result = pdoQueryStm($db_lnk, 'SELECT * FROM cards_labels_listing WHERE  card_id = ?', $obj_val_arr);
...
function update_query($table_name, $id, $r_resource_cmd, $r_put, $comment = '', $activity_type = '', $foreign_ids = '')
{
    ...
    if (!empty($table_name) && !empty($id)) {
        ...
        pdoQueryStm($db_lnk, $query, $values);
    }
    if (empty($response)) {
        $response = 'Success';
    }
...
function json_response($table_name, $r_resource_vars)
{
    ...
    $stm = $db_lnk->prepare($sql);

    if ($stm->execute($pg_params)) {
